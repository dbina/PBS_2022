...
[ fname, fpath ] = uigetfile( ['C:\DataVB\Progs\PBS_sims_2022\','new_overlaps_exp_alt.txt'], 'load overlaps' )
disp( fname )
if fname == 0 
...
% switch ApcF-beta84 to regular beta84 to 
index( index == 6 ) = 2;
...
% % reduce files to 1 rod
% mRe = mRe(1:54, :);
% index = index(1:54); 

% % reduce files to core
% mRe = mRe((6*54 + 1):end, :);
% index = index((6*54 + 1):end ); 
...
% hold on
% plot3( mRe( index > 5 & index < 9,2), mRe(index > 5 & index < 9,3), mRe(index > 5 & index < 9,4), 'rsq' )
% 
...
% % plot3(  mRe(1:54,2), mRe(1:54,3), mRe(1:54,4), 'k+' )
% % hold on
% % plot3(  mRe(46:54,2), mRe(46:54,3), mRe(46:54,4), 'rsq' )
% % grid on
...
fpath = 'C:\DataVB\Progs\PBS_sims_2022\';
% assign dipole moments based on index
for ii = 1:length( index )
    mRe(ii,1) = 12;
    switch index(ii)
        case 9
             mRe(ii,1) = 0.0; %[D]%3.4634;
%              mRe(ii,1) = 0;
        case 3        
            mRe(ii,1) = 13;            
        case 4        
...
% switch off carotenoids (set i-th dipole = 0)
% mRe(397,1) = 0;
% mRe(398,1) = 0;
% mRe(399,1) = 0;
% mRe(400,1) = 0;
...
% csvwrite( 'C:\Data\2022_01\pbs_rev\carVij.txt', Kij( :, end-3:end ) );
% VH = tril( Kij );
% VH = reshape( VH, [], 1 );
% VH = nonzeros( VH );
% VH = sqrt(VH.^2);
...
% Kcara = Kij( :, acccol );
% tcara = 1./Kcara;
% [x1,x2,x3] = find( tcara > 0 & tcara < 180 )
% 
% for iii = 1:size( x1, 1 )
%     
%     disp( [ 'D row: ', num2str(x1(iii)), ', A col: ', num2str(x2(iii)), ', t[ps] = ', num2str( tcara( x1(iii), x2(iii) ) ) ] )
%     
% end
...
%startpts = [45:54];
% startpts = [111, 114, 117, 112, 109, 110 ] ; %PBS-OCP 3x b155, 1x b84, 2xa84
%startpts = [219, 222, 225, 217, 220, 218 ]; % down down
startpts = [291, 294, 297, 289, 292, 290 ]; % up-up
% startpts = [46:54];
% startpts = 3*54+1; % down-down
% startpts = [46,47,48];
% startpts = [48,225,114];
% startpts = [60,114,168];  % down down
% startpts = 54+54+54+(1:9);% down down
% startpts = 54+54+54+1;% down down

%startpts = 54+54+72+54+(1:9); % up-up
% startpts = 54+54+72+54+1; % up-up
% startpts = [132,186,240];  % up up
% startpts = ceil( rand(10,1)*54 ) ;
...
% %#####################################
% % Gillespie part
% disp( 'running Gillespie model of PBS... ')
% %1. modify rate matrix
% gnd = ones( 1, size( K,2 )+1  )/1500;  % ground state
% gnd( index == 4 ) = 1/900;
% gnd( index == 9 ) = 1/3.3;
% 
% Kgill = [ [Ktest, zeros( size(Ktest,1), 1 )]; gnd ];
% 
% T = [(0:0.1:10),(11:1:3000)]';
% 
% for kk = 1:length( startpts )
%     tic
%     [Z, g] = gillespie( Kgill, T, 30, 1, startpts(kk) );
% %     g
%     toc
%     Z = Z(:,2:end);   
%     for nn = 1:9
%         
%         G(:,nn) = [ nn; sum( Z(:, index == nn ), 2 ) ];
%         
%     end
%     
%     semilogx( T, G(2:end, :), '-' )
...
%     gillout = [ [999;T], G];
% 
%     zout = [ [999;T], [index';Z(:,1:end-1)] ];
%     save(['C:\Data\2021_09\gill_kin_classes_dd_exc',num2str( startpts(kk) ),'.txt'], 'gillout', '-ascii') 
%     save(['C:\Data\2021_09\gill_kin_all_dd_exc',num2str( startpts(kk) ),'.txt'], 'zout', '-ascii') 
%     
% end
% 
% % return
% % figure
% % plot3( mRe( :, 2), mRe( :, 3), mRe( :, 4), 'sq', 'MarkerFaceColor', [0.8,0.8,0.8],'MarkerEdgeColor', [0.8,0.8,0.8] )
% % hold on
% % set(gca,'DataAspectRatio', [1 1 1])
% % 
% sv = zeros( size(Kgill,1), 1) ; % states vector
% a = unique( g(:,1) );
% for ii = 1:length(a)
%     
%     h = g( g(:,1) == a(ii) , 2 );
% %     disp( [num2str(ii), ', ', num2str( a(ii)) ] )
% %     disp( h ) 
%     h = unique(h);
% 
% %     for mm  = 1:length(h)-1
% %         
% %         plot3( mRe( h(mm), 2), mRe( h(mm), 3), mRe( h(mm), 4), '.', 'MarkerFaceColor', [ii/length(a),0,1-ii/length(a)],...
% %             'MarkerEdgeColor', [ii/length(a),0,1-ii/length(a)] )
% %         
% %     end
%     sv( h ) = sv(h) + 1;
%     
...
% figure
% n = 30;
% pc = [1, 0.90, 0.70, 0.5] * n;
% sv = sv(1:end-1);
% x = 1:length( sv );
% length( sv )
% size( mRe )
% 
% plot3( mRe( :, 2), mRe( :, 3), mRe( :, 4), 'sq','MarkerFaceColor', [0.7, 0.7, 0.7  ],...
%                                                 'MarkerEdgeColor', [ 0.7, 0.7, 0.7 ])
% hold on
% for ii = 1:length( pc )-1
%     
%     c = x( sv <= pc(ii) & sv > pc(ii+1) );
%     plot3( mRe( c, 2), mRe( c, 3), mRe( c, 4), 'o', 'MarkerSize', 2+2*length(pc)-2*ii,...
%                            'MarkerFaceColor', [ 1-(ii-1)/6, 0, (ii-1)/6 ],...
%                            'MarkerEdgeColor', [ 1-(ii-1)/6, 0, (ii-1)/6 ])
%     
% end
% set(gca,'DataAspectRatio', [1 1 1])
% 
...
%     N = length( t );
    ...
    kin_name = 'up_up_cpc13_apc15_tem15_bmean_4TEM';         
%     [kinn, fpath] = uiputfile( [fp0, kin_name,'.txt'], 'file for kinetics' )     
%     kin_name = kinn(1:end-4);
    ...
    disp( 'time to save .csv:' )
    csvtime = toc;
    disp([ num2str( csvtime / 60),' minutes'])
    tempy =  y( timepick, :);
...
%--------------------------------------------------------------------
function [g, trk] = gillespie( Ktest, T, nruns, gettrack, startstate )
% runs the Gillespie model using the 
% matrix of rate constants, Ktest
% Ktest is the same as used for the ode function
% if gettract == 1 then all visited states are recorded and saved
%
nstates = size( Ktest, 1);

T = T(:); % output time
S = zeros( length(T), nstates); % initialize states with zeros
nn = 1;
...
for jj = 1:nruns
    ...
    curstate = startstate; % current initial state of the system   
    S(:, curstate) = S(:, curstate) + 1; % if no event occurs, system remains in current state till the end
    que = true;
    ii = 2;
    t = 0; % start at time zero
    
    if gettrack == 1
        track( nn, : ) = [ jj, curstate ];
        nn = nn + 1;
    end
    
    while que

        t = t + 1/abs(Ktest( curstate, curstate ))*log( 1 / rand(1) ); % time-to-next-event
        c = [ Ktest( :, curstate ), (1 : nstates)' ]; % column from rate matrix
%         b = c( c(:,1) ~= 0, :); % eliminate impossible (kij = 0) step directions
%        b = c( c(:,1) > 10*eps, :); % eliminate impossible (kij ~ 0) step directions
        b = c( c(:,1) > 1e-9, :); % eliminate impossible (kij ~ 0) step directions
        a = b( b(:,2) ~= curstate, : ); % these are valid directions; eliminate the no-event-option
...
%         min( a(:,1) )
        ...
        a(:,1) = cumsum( a(:,1) ); % cumulative rate      
        a = [ [ 0,0 ]; a ]; 

        r2 = rand(1) * max( a(:,1) );      

%         aa1 = length( a(:,1) )
%         aa2 = length( unique( a(:,1) ) )
%         find( a(2:end,1) - a(1:end-1,1) == 0 )       
%         if aa1 ~= aa2
%             
%             disp( a(:,1) )
%             figure
%             
%             plot( 1:length( a(:,1) ), a(:,1) )
%             
%             a(2:end,1) - a(1:end-1,1)
%             
%         end
...
%         end
%         newstate = zz;
         newstate = interp1( a(:,1), (1:length(a(:,1)) )', r2 );
%         ceil( newstate )
        S( T > t, curstate ) = S( T > t , curstate ) - 1; % current state is emptied for the rest of time T or until occupied again
        curstate = a( ceil( newstate ), 2); % pick the current state
        S( T > t, curstate ) = S( T > t, curstate ) + 1; % a new state is occupied; if nothing else happens, system stays here
               
        if gettrack == 1
            track(nn, : ) = [jj, curstate] ;% keeps tract of states visited
            nn = nn + 1;
        end
        
        if curstate == nstates % when given state is populated, current simulation ends
            que = false;
        end       
        if t > max( T ) 
            que = false;
